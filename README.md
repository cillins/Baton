...
Apple 的配置描述文件明确限制再分发，并会在安装 3 天后自动移除，因此 Baton 不会把 PacketLogger 或该 `.mobileconfig` 放进应用及公开 DMG。设置页提供“选择并安装”入口；Baton 会读取所选配置并确认其确实是 `Bluetooth Logging for macOS`，避免误装 iOS 配置，然后再交给系统设置确认安装。Apple 当前标为 macOS 的在线下载可能返回 iOS Payload，因此 Baton 不提供该下载入口。
